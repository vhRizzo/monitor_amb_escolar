...
struct DSM501a2_5SensorStore {
  volatile uint32_t last_interrupt{0};
  volatile uint32_t on_time{0};
  volatile bool last_level{false};
  ISRInternalGPIOPin *pin;

  static void gpio_intr(DSM501a2_5SensorStore *arg);
};

class DSM501a2_5Sensor : public sensor::Sensor, public PollingComponent {
public:
  DSM501a2_5Sensor() : PollingComponent (60000) {}
...
protected:
  ...
  DSM501a2_5SensorStore store_{};
  uint32_t last_update_;
};

void DSM501a2_5Sensor::setup() {
  ESP_LOGCONFIG(TAG, "Setting up DSM501a 2.5 um Sensor '%s'...", this->get_name().c_str());
  GPIOPin *aux = new GPIOPin (36, INPUT);
  this->set_pin(aux);
  this->pin_->setup();
  ...
  this->pin_->attach_interrupt(DSM501a2_5SensorStore::gpio_intr, &this->store_, CHANGE);
}
void DSM501a2_5Sensor::dump_config() {
  LOG_SENSOR("", "DSM501a 2.5 um Sensor", this);
  LOG_PIN("  Pin: ", this->pin_);
  LOG_UPDATE_INTERVAL(this);
}
void DSM501a2_5Sensor::update() {
  ...
  ESP_LOGD(TAG, "'%s' Got PM 2.5 um = %.2f", this->get_name().c_str(), value);
...
float DSM501a2_5Sensor::get_setup_priority() const { return setup_priority::DATA; }

void ICACHE_RAM_ATTR DSM501a2_5SensorStore::gpio_intr(DSM501a2_5SensorStore *arg) {
